...
from vehicleform.models import Brand, Vehicle
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.WARNING(
            "Flushing `Brand` & `Vehcile`..."))
        Vehicle.objects.all().delete()
        Brand.objects.all().delete()

        with open(os.path.join("vehicleform", "management", "dataset.json"), "r") as file:
            data = json.load(file)
            file.close()
            ...
            self.stdout.write(self.style.WARNING(
                f"{len(saved_brands)} brand(s) were saved!"))
            ...
            for brand, vehicles in data.items():
                for vehicle_name in vehicles:
                    ...
                    if vehicle_brand_pk:
                        vehicle_list.append(
                            Vehicle(vehicle_name=vehicle_name, vehicle_brand=Brand(pk=vehicle_brand_pk)))

            saved_vehicle = Vehicle.objects.bulk_create(vehicle_list)
            self.stdout.write(self.style.WARNING(
                f"{len(saved_vehicle)} vehicle(s) were saved!"))

            self.stdout.write(self.style.SUCCESS(
                "Successfully loaded data for `brands` & `vehciles`."))
